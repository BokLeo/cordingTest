...
function 이진변환반복하기(s:string) {
	let rotationCnt : number = 0;
	let zeroCnt : number = 0;

	// 0을 제거하는 함수 => return [제거된 0의 갯수, 제거된 값]
	const deleteZero = (param:string) : [number, string] => {
	...
	// 이진법 변환 함수
	const toBinary = (param:string) : string => {
		return param.length.toString(2);
	}

	const logic = (param:string) : string => {
...
function test(testCase: string, expected: [number, number]): void {
